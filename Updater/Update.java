...
public void update() {
	...
	try {
		URL website = new URL("https://raw.githubusercontent.com/frychicken/PoolTableSimulator/master/makeJar/PoolSimulator.jar");
		ReadableByteChannel ok = Channels.newChannel(website.openStream());
		FileOutputStream okay = new FileOutputStream(System.getProperty("user.dir")+"/PoolSimulator.jar");
	...
	try {
		r.exec("java -jar PoolSimulator.jar " + fail);
	} catch (IOException e) {
		e.printStackTrace();
	}
